...
static HAL_StatusTypeDef max7219_transmit(uint8_t _address, uint8_t _data) {
	...
	HAL_GPIO_WritePin(max7219_handle->spi_ncs_port, max7219_handle->spi_ncs_pin,
			GPIO_PIN_RESET);
...
HAL_StatusTypeDef max7219_init(MAX7219_Handle_TypeDef *_max_7219_handle) {
	...
	max7219_status = max7219_transmit(SHUTDOWN_REG_BASE,
	SHUTDOWN_REG_SHUTDOWN_MODE);
	...
	// Enable MAX7219
	max7219_status = max7219_transmit(SHUTDOWN_REG_BASE,
	SHUTDOWN_REG_NORMAL_MODE);
	if (max7219_status != HAL_OK)
		return max7219_status;

	// Set scan limit to number of digits
	max7219_status = max7219_transmit(SCAN_LIMIT_REGG_BASE,
			max7219_handle->digits_count - 1);
	if (max7219_status != HAL_OK)
		return max7219_status;
 ...
 */
HAL_StatusTypeDef max7219_display_no_decode(uint8_t _digit_index,
		uint8_t _digit_value) {
	HAL_StatusTypeDef max7219_status = HAL_OK;
	...
	// Display value
	max7219_status = max7219_transmit(digits_registers[_digit_index],
			_digit_value);
	if (max7219_status != HAL_OK)
		return max7219_status;
